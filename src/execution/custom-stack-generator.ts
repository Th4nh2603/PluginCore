...
import type { AuthenticationExecutor } from "./capabilities/authentication.js";
...
const customStackReadme = (config: RepoConfig, authentication?: AuthenticationExecutor): string => {
...
export const generateCustomStack = async (root: string, config: RepoConfig, runner: GeneratorRunner, authentication?: AuthenticationExecutor, runCommands = true): Promise<void> => {
  ...
  const backend = componentId(config.composition.stack.backend ?? config.composition.stack["backend-framework"], "express");
  const orm = componentId(config.composition.stack.orm, "prisma");
  const mcpEnabled = hasMcpCapability(config);
  ...
  if (hasWeb) await writeFiles(monorepo ? path.join(root, "apps/web") : root, authentication === undefined
    ? frontendFiles(monorepo ? `@${config.project.name}/web` : config.project.name, frontend)
    : authentication.frontendFiles(`@${config.project.name}/web`, frontend));
  if (hasApi) {
    await writeFiles(apiRoot, authentication === undefined
      ? backendFiles(monorepo ? `@${config.project.name}/api` : config.project.name, backend, orm, undefined, mcpEnabled)
      : authentication.backendFiles(`@${config.project.name}/api`, backend, orm, mcpEnabled));
...
export const applyCustomAuthentication = async (root: string, config: RepoConfig, runner: GeneratorRunner, authentication: AuthenticationExecutor): Promise<void> => {
  if (config.project.type !== "monorepo" || selectGenerationStrategy(config) !== "custom") {
    throw new Error("Custom authentication requires a Custom Monorepo stack.");
  }
  const frontend = componentId(config.composition.stack.frontend ?? config.composition.stack["frontend-library"], "react");
  const backend = componentId(config.composition.stack.backend ?? config.composition.stack["backend-framework"], "express");
  const orm = componentId(config.composition.stack.orm, "prisma");
  ...
  await writeFiles(path.join(root, "apps/web"), authentication.frontendFiles(`@${config.project.name}/web`, frontend));
  await writeFiles(apiRoot, authentication.backendFiles(`@${config.project.name}/api`, backend, orm, hasMcpCapability(config)));
  const environment = `DATABASE_URL=postgresql://app:app@localhost:5432/app\nWEB_ORIGIN=http://localhost:5173\nPORT=3001\n${authentication.environment()}`;
  await writeFiles(apiRoot, { ".env": environment, ".env.example": environment.replace(/JWT_SECRET=.+/, "JWT_SECRET=replace-with-at-least-32-random-characters") });
  await writeFiles(root, { "README.md": customStackReadme(config, authentication) });

  await runner.run(pnpm, ["install"], root);
  if (orm === "prisma") await runner.run(pnpm, ["--filter", "./apps/api", "exec", "prisma", "generate"], root);
};
